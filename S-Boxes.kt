...
fun main(args: Array<String>) {
    ...
    val inputBinary1 = Integer.toBinaryString(java.lang.Long.parseLong(inputHex.substring(0, 4), 16).toInt())
    val inputBinary2 = Integer.toBinaryString(java.lang.Long.parseLong(inputHex.substring(4, 8), 16).toInt())
    val inputBinary3 = Integer.toBinaryString(java.lang.Long.parseLong(inputHex.substring(8, 12), 16).toInt())

    val binary = handleLeftZeros(inputBinary1) + handleLeftZeros(inputBinary2) + handleLeftZeros(inputBinary3)

    var hexOut = ""

    for (i in 0 until  8){

        val str = binary.substring(i * 6, i * 6 + 6)
        val row = ("" + str[0] +  str[5]).toInt(2).toString(10).toInt()
        val col = str.substring(1, 5).toInt(2).toString(10).toInt()

        hexOut += Integer.toHexString(boxes[i][row][col])

    }

    println(hexOut.toUpperCase())

}

fun handleLeftZeros(string: String) : String{

    var zeros = ""

    for (i in 0 until (16 - string.length)){
        zeros += "0"
    }
